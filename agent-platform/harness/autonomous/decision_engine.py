...
class OpenAIAutonomousDecisionEngine:
    ...
    async def decide(
        ...
        *,
        ...
        completion = await self._request_completion_with_retries(prompt_payload)
    ...
    async def _request_completion_with_retries(self, prompt_payload: dict[str, object]):
        for attempt in range(1, self._MAX_COMPLETION_ATTEMPTS + 1):
            try:
                return await self._request_completion(prompt_payload)
            except self._retryable_error_types() as exc:
                if attempt >= self._MAX_COMPLETION_ATTEMPTS:
                    message = str(exc).strip() or exc.__class__.__name__
                    raise RuntimeError(
                        "Autonomous decision generation failed after "
                        f"{self._MAX_COMPLETION_ATTEMPTS} attempts: {message}"
                    ) from exc
                await asyncio.sleep(self._retry_delay_seconds(attempt))
        raise RuntimeError("Autonomous decision generation failed without producing a completion.")

    async def _request_completion(self, prompt_payload: dict[str, object]):
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "Treat Stimpact SDK/bootstrap/instrumentation code as protected. Do not delete, stub, or replace "
                            "Stimpact initialization or handled-error capture with local no-op fallbacks unless the incident is "
                            "explicitly about Stimpact SDK/telemetry/bootstrap behavior and your evidence shows that code is the root cause. "
                            "If verification is blocked by missing Stimpact wiring, prefer restoring the intended integration over disabling observability. "
    ...
    def _retry_delay_seconds(self, attempt: int) -> float:
        return self._INITIAL_RETRY_DELAY_SECONDS * (2 ** (attempt - 1))
