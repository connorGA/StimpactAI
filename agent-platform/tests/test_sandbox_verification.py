...
def test_local_sandbox_runner_reproduces_applies_patch_and_verifies(tmp_path: Path) -> None:
    ...
    result = runner.run(
        ...
        commands=SandboxCommandSet(
            ...
            reproduce_command="python3 -c \"from pathlib import Path; import sys; sys.exit(1 if \\\"old\\\" in Path('app.py').read_text() else 0)\"",
...
def test_local_sandbox_runner_fails_when_reproduce_command_is_unavailable(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    target_file = repo_root / "app.py"
    target_file.write_text("VALUE = 'old'\n", encoding="utf-8")
    ...
    result = runner.run(
        ...
        commands=SandboxCommandSet(
            install_command=None,
            reproduce_command="missing-command-for-sandbox",
    ...
    assert result.reproduction_succeeded is False
    assert result.patch_applied is False
    assert result.verification_succeeded is False
    assert "could not reproduce" in result.summary
